<?php
$vporid=intval($_GET['uid']);

$dbc=mysql_connect("127.0.0.1","root","mint") or die (mysql_error());

$dbselect=mysql_select_db("askdev")or die (mysql_error());

$sud=mysql_query("SELECT * 
FROM  `users` 
WHERE  `uemail` =  '$vporid'
LIMIT 0 , 30");
$fsud=mysql_fetch_object($sud);
 define("fname",$fsud->ufname);
 define("sname",$fsud->usname);
 define("email",$fsud->uemail);
 define("bday",$fsud->ubday);
 define("sex",$fsud->usex);
 define("upic",$fsud->upic);
 define("ucoverpic",$fsud->ucoverpic);
$uid=$fsud->uid;
...
$vpos=mysql_query("select * from users where uid=$vporid")or die(mysql_error());
$vposNUM=mysql_num_rows($vpos);
if($vposNUM==0){
echo"<script>location.href = '404.php';</script>";
}
$vposf=mysql_fetch_assoc($vpos);
?>
        <div class="profile-cont">
        <div class="cover">
              <img src="data:image/jpeg;base64,<?php echo base64_encode( $vposf['ucoverpic']); ?>" width="100%" height="280px" />
...
?>
            <img src="data:image/jpeg;base64,<?php echo base64_encode($vposf['upic']); ?>" id="ppics" width="140" height="140" />
...
"><?php echo $vposf['ufname']." ".$vposf['usname'] ?></a></h3>
            </div>
             <div class="h-p">
        <h4>
اكتب ما في بالك
        </h4>
  <form action="vporofile.php?uid=<?php echo $vporid ?>"  method="post" enctype="multipart/form-data">
...
FROM  `post` where userpost=$vporid
ORDER BY postid  desc 
LIMIT 0 , $nump  ");
while($fp=mysql_fetch_object($selectpost)){
    $upid=$fp->userpost;
    
$suser=mysql_query("select * from users where uid=$vporid")or die(mysql_error());
...
<?php
    if($uid == $upid){
    echo '          
    <form action="vporofile.php?uid='.$vporid.'" method="POST">
...
FROM  `like` 
WHERE uli=$vporid AND pid=$pidl")or die(mysql_error());
...
<div class="comments"  >
                ...
                <form action="vporofile.php?uid=<?php echo $vporid ?>" method="post">
...
?>
 ...
 <div class="cont-home-left">
    <?php
  $sefo=mysql_query("select * from follow where `u-f-id`=$uid AND  `u-f-id-w`=$vporid")or die(mysql_error());
if($sefon=mysql_num_rows($sefo)==0){
echo'    <button id="follow"style="
    background: #27B383;
    border: 1px;
    width: 25%;
    height: 8%;
    ...
    font-size: 120%;
    text-align: right;
" onclick="follow()" name="follow">متابعه+</button>';
...
echo'    <button id="follow" style="
    ...
    text-align: right;
    padding: 2%;
" onclick="unfollow()" name="follow">اللغاء المتابعه+</button>';
...
?>
       ...
       function follow(){
             ...
             $('#re').load ('follow.php?uid=<?php echo $uid; ?>&ufid=<?php echo $vporid; ?>');

       };   function unfollow(){

             $('#re').load ('follow.php?uid=<?php echo $uid; ?>&ufid=<?php echo $vporid; ?>&fun=unf');
...
<script>(function(d, s, id) {
  ...
  js.src = "//connect.facebook.net/en_US/sdk.js#xfbml=1&version=v2.5&appId=794304294015300";
  fjs.parentNode.insertBefore(js, fjs);
}(document, 'script', 'facebook-jssdk'));</script>
     تابعني على فيسبووك
     <div class="fb-follow" data-href="https://www.facebook.com/faroukmhammad" data-layout="button_count" data-show-faces="false"></div>
